...
@section('content')
    <div class="row">
        <div class="col-12 col-xl-8 mb-4 mb-xl-0">
            ...
            <h3 class="font-weight-bold">Edit Wisata</h3>
        </div>
            ...
            <div class="col-12 grid-margin mt-3 stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST" action="{{ route('admin.wisata.update', $wisata->id) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                      <div class="form-group">
                        <label for="exampleInputName1">*Nama Tempat</label>
                        <input type="text" name="nama_tempat" value="{{ old('nama') ?? $wisata->nama_tempat }}" class="form-control" id="exampleInputName1" placeholder="Name" required>
                      </div>
                      <div class="form-group">
                        <label for="exampleInputEmail3">*Lokasi</label>
                        <input type="text" name="lokasi" value="{{ old('lokasi') ?? $wisata->lokasi }}" class="form-control" id="exampleInputEmail3" placeholder="Lokasi" required>
                      </div>
                      <div class="form-group">
                            <label for="ticketPrice">*Harga Tiket Masuk</label>
                            <input name="harga_tiket" value="{{ old('harga_tiket') ?? $wisata->harga_tiket }}" type="text" id="ticketPrice" class="form-control" value="50000" />
                      </div>
                      <div class="form-group">
                        <label for="exampleInputPassword4">*Fasilitas</label>
                        <textarea type="text" name="fasilitas" row="3" class="form-control" id="exampleInputPassword4" placeholder="*WC UMUM dll">{{ old('fasilitas') ?? $wisata->fasilitas }}</textarea>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <textarea name="deskripsi" class="form-control" id="exampleTextarea1" rows="4">{{ old('deskripsi') ?? $wisata->deskipsi }}</textarea>
                      </div>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="d-flex">
                            <img id="preview1" class="{{ $wisata->banner1 != null ? 'd-block' : '' }} imgPreview" src="{{ $wisata->banner1 }}" />
                            <img id="preview2" src="{{ $wisata->banner2 }}" class="{{ $wisata->banner2 != null ? 'd-block' : '' }} imgPreview" />
                            <img id="preview3" src="{{ $wisata->banner3 }}" class="{{ $wisata->banner3 != null ? 'd-block' : '' }} imgPreview" />
                        </div>
                    </div>
                  </div>
                </div>
